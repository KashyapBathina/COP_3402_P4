...
code_seq gen_code_assign_stmt(assign_stmt_t *stmt) {
    ...
    code_seq_concat(&ret, code_pop_stack_into_reg(V0, 0));
    code_seq_concat(&ret, code_utils_compute_fp(T9, stmt->idu->levelsOutward));
    unsigned int offset_count = id_use_get_attrs(stmt->idu)->offset_count;
    code_seq_add_to_end(&ret, code_swr(T9, offset_count, V0));
    return ret;
}

// Generate code for a call statement
code_seq gen_code_call_stmt(call_stmt_t *stmt) {
    // Placeholder for call statement code generation
    bail_with_error("Call statements are not supported in this version of SPL.");
...
code_seq gen_code_if_stmt(if_stmt_t *stmt) {
    ...
    code_seq_concat(&ret, code_pop_stack_into_reg(V0, 0));
    code_seq_add_to_end(&ret, code_beq(V0, 0, then_len + 1));
...
code_seq gen_code_while_stmt(while_stmt_t *stmt) {
    ...
    code_seq_concat(&ret, code_pop_stack_into_reg(V0, 0));
    code_seq_add_to_end(&ret, code_beq(V0, 0, body_len + 1));
...
code_seq gen_code_read_stmt(read_stmt_t *stmt) {
    code_seq ret = code_seq_singleton(code_rch(V0, 0));
    code_seq_concat(&ret, code_utils_compute_fp(T9, stmt->idu->levelsOutward));
    unsigned int offset_count = id_use_get_attrs(stmt->idu)->offset_count;
    code_seq_add_to_end(&ret, code_swr(T9, offset_count, V0));
    return ret;
...
code_seq gen_code_print_stmt(print_stmt_t *stmt) {
    code_seq ret = gen_code_expr(&(stmt->expr));
    code_seq_concat(&ret, code_pop_stack_into_reg(A0, 0));
    code_seq_add_to_end(&ret, code_pint(A0, 0));
    return ret;
}
...
code_seq gen_code_expr(expr_t *expr) {
    switch (expr->expr_kind) {
        ...
        default:
            bail_with_error("Unexpected expr_kind_e (%d) in gen_code_expr", expr->expr_kind);
    }
    return code_seq_empty(); // should never reach here
}
...
code_seq gen_code_negated_expr(negated_expr_t *expr) {
    ...
    code_seq_concat(&ret, code_pop_stack_into_reg(AT, 0));
    code_seq_add_to_end(&ret, code_neg(AT, AT, 0));
    code_seq_concat(&ret, code_push_reg_on_stack(AT, 0));
    return ret;
}
...
code_seq gen_code_op(token_t op) {
    switch (op.code) {
        case plussym:
            return code_seq_singleton(code_add(V0, V0, AT, 0));
        case minussym:
            return code_seq_singleton(code_sub(V0, V0, AT, 0));
        case multsym:
            return code_seq_singleton(code_mul(V0, V0, AT, 0));
        case divsym:
            return code_seq_singleton(code_div(V0, V0, AT, 0));
        default:
...
code_seq gen_code_number(int value) {
    unsigned int global_offset = literal_table_lookup("", value);
    code_seq ret = code_seq_singleton(code_lwr(V0, GP, global_offset));
    return code_seq_concat(&ret, code_push_reg_on_stack(V0, 0));
}

// Generate code for an identifier
code_seq gen_code_ident(ident_t *id) {
    code_seq ret = code_utils_compute_fp(T9, id->idu->levelsOutward);
    unsigned int offset_count = id_use_get_attrs(id->idu)->offset_count;
    ret = code_seq_add_to_end(&ret, code_lwr(V0, T9, offset_count));
    return code_seq_concat(&ret, code_push_reg_on_stack(V0, 0));
}
